...
export function DroneCard(props) {
  ...
  const clickSetDrone = () =>{
    setSelectedCategory((prevOrderList) => {
      const updatedOrderList = [...prevOrderList];
      updatedOrderList[props.category-1] = {
  ...
  return(
    <Drones onClick={clickSetDrone} borderColor={(selectedCategory[props.category-1].itemId === props.item.id) ? "red" : '#808080'} boxShodow={(selectedCategory[props.category-1].itemId === props.item.id) ? true : false}>
